...
from modelos import Equipo, Partido, Apuesta, Usuario 
...
class VentanaBienvenida(wx.Dialog):
    ...
    def _construir_ui(self):
        ...
        descripcion = (
            ...
            " Viale Vladimir, Morales German, Ventura Brandon Comision 4"   
            )
...
class LigaCoinApp(wx.Frame):
    ...
    def __init__(self):
        ...
        self._imagenes_equipos = wx.ImageList(32, 32)
        self._mapa_logos = {}

        base_dir = os.path.dirname(os.path.abspath(__file__))

        for nombre_equipo in self.NOMBRES_EQUIPOS:
            try:
                ruta_logo = os.path.join(base_dir, "imagenes", f"{nombre_equipo}.png")
                img = wx.Image(ruta_logo, wx.BITMAP_TYPE_PNG)
                img = img.Rescale(32, 32)
                bmp = wx.Bitmap(img)
                idx = self._imagenes_equipos.Add(bmp)
                self._mapa_logos[nombre_equipo] = idx
            except Exception as e:
                print(f"Error cargando logo {nombre_equipo}: {e}")
                self._mapa_logos[nombre_equipo] = -1

      
        self._lista_partidos.AssignImageList(self._imagenes_equipos, wx.IMAGE_LIST_SMALL)
    ...
    def _construir_ui(self):
        ...
        sizer_header.Add(self._lbl_saldo, 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 18)
        ...
        self._lista_partidos.InsertColumn(0, "", width=40)
        self._lista_partidos.InsertColumn(1, "Local", width=130)
        self._lista_partidos.InsertColumn(2, "", width=40)
        self._lista_partidos.InsertColumn(3, "Visitante", width=130)
        self._lista_partidos.InsertColumn(4, "Fecha", width=80)
        self._lista_partidos.InsertColumn(5, "Estado", width=80)
        self._lista_partidos.InsertColumn(6, "Marcador", width=65)
        self._lista_partidos.Bind(wx.EVT_LIST_ITEM_SELECTED, self._on_seleccionar_partido)

        self._btn_simular = wx.Button(panel_izq, label="▶️  Iniciar Simulación", size=(-1, 34))
        ...
        btn_reiniciar = wx.Button(panel_izq, label="🔄  Reiniciar Temporada", size=(-1, 30))
        ...
        self._lbl_partido_actual = wx.StaticText(panel_der, label="(Seleccioná un partido de la lista)")
        self._lbl_partido_actual.SetForegroundColour(wx.Colour(80, 80, 160))
        self._lbl_partido_actual.SetFont(wx.Font(9, wx.FONTFAMILY_DEFAULT,
                                                 wx.FONTSTYLE_ITALIC, wx.FONTWEIGHT_NORMAL))

        lbl_ganador = wx.StaticText(panel_der, label="Apostá al ganador:")
        self._choice_equipo = wx.Choice(panel_der, choices=["— Elegí un equipo —"])
        ...
        self._lbl_msg_apuesta.SetFont(wx.Font(9, wx.FONTFAMILY_DEFAULT,
                                              wx.FONTSTYLE_ITALIC, wx.FONTWEIGHT_NORMAL))
        ...
        self._lista_historial = wx.ListCtrl(panel_der, style=wx.LC_REPORT | wx.BORDER_SUNKEN)
        ...
        btn_limpiar = wx.Button(panel_der, label="🗑  Limpiar Historial", size=(-1, 30))
        ...
        lbl_footer.SetFont(wx.Font(8, wx.FONTFAMILY_DEFAULT, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL))
    ...
    def _actualizar_lista_partidos(self):
        ...
        for p in self._partidos:
            local = p.get_local().get_nombre()
            visitante = p.get_visitante().get_nombre()

            logo_local = self._mapa_logos.get(local, -1)
            logo_visitante = self._mapa_logos.get(visitante, -1)

            
            idx = self._lista_partidos.InsertItem(
                self._lista_partidos.GetItemCount(),
                "",
                imageIndex=logo_local
            )

          
            self._lista_partidos.SetItem(idx, 1, local)

           
            item_visitante = wx.ListItem()
            item_visitante.SetId(idx)      
            item_visitante.SetColumn(2)    
            item_visitante.SetImage(logo_visitante)
            self._lista_partidos.SetItem(item_visitante)

            
            self._lista_partidos.SetItem(idx, 3, visitante)
            self._lista_partidos.SetItem(idx, 4, p.get_fecha())
            self._lista_partidos.SetItem(idx, 5, p.get_estado())
            self._lista_partidos.SetItem(idx, 6, p.get_marcador())
    ...
    def _on_apostar(self, event):
        ...
        self._lbl_msg_apuesta.SetLabel(f"✅ Apostaste {cantidad} LC a {equipo_apostado}.")
        self._txt_cantidad.SetValue("")

    def _on_iniciar_simulacion(self, event):
        if self._partido_sel is None:
            wx.MessageBox("Seleccioná un partido para simular.", "Aviso", wx.OK | wx.ICON_INFORMATION)
            return

        if self._partido_sel.get_estado() == Partido.FINALIZADO:
            wx.MessageBox("Ese partido ya finalizó.", "Aviso", wx.OK | wx.ICON_INFORMATION)
            return

        if self._timer is not None and self._timer.IsRunning():
            wx.MessageBox("Ya hay una simulación en curso.", "Aviso", wx.OK | wx.ICON_INFORMATION)
            return
    ...
    def _on_tick_simulacion(self, event):
        ...
        if minuto >= 90:
            ...
            self._actualizar_lista_partidos()
            self._lbl_minuto.SetLabel(f"✅ Partido finalizado  |  {p.get_marcador()}")
    ...
    def _resolver_apuestas_del_partido(self, partido):
        ...
        for apuesta in self._usuario.get_apuestas():
            if (apuesta.get_partido() is partido and apuesta.get_resultado() == Apuesta.PENDIENTE):
                apuesta.resolver()
    ...
    def _actualizar_historial(self):
        ...
        for a in self._usuario.get_apuestas():
            idx = self._lista_historial.InsertItem(self._lista_historial.GetItemCount(), str(a.get_partido()))
            self._lista_historial.SetItem(idx, 1, a.get_equipo_apostado().get_nombre())
